(function () {
  ...
  const renderVerse = (lesson, verse) => {
    ...
    return `<details class="verse" id="verse-${n}"><summary><span class="verse-no">${n}</span><span class="verse-text"><span>${escape(first)}</span><span>${escape(second)}</span></span><span class="chev">⌄</span></summary><div class="verse-body"><p class="quick"><strong>${u.oneLine} :</strong> ${escape(brief)}</p><h4>${u.simple}</h4><p>${escape(simple)}</p><h4>${u.detailed}</h4><p>${escape(detail)}</p><div class="details-grid"><section><h4>${u.terms}</h4><ul class="terms">${lesson.terms.map(t=>`<li>${escape(t)}</li>`).join('')}</ul></section><section><h4>${u.example}</h4><p>${escape(lesson.example)}</p><p class="source-line">${u.source}</p></section><section><h4>${u.warning}</h4><p>${escape(lesson.warning)}</p></section><section><h4>${u.exercise}</h4><p>${escape(lesson.exercise)}</p></section></div><p class="source-line"><strong>${u.references} :</strong> ${escape(lessonRefs)}</p><button class="copy" data-copy="${escape(lineTitle(verse))}">${u.copy}</button></div></details>`;
  };
