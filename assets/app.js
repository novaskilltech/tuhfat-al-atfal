(function () {
  ...
  const cardCanvas = (card, index) => {
    const canvas=document.createElement('canvas'), ctx=canvas.getContext('2d'), w=1400, h=2200, pad=110;
    ...
    const font=(weight,size)=>`${weight} ${size}px ${rtl?'Amiri, "Noto Naskh Arabic", serif':'Inter, Arial, sans-serif'}`;
    ...
    let y=254; y=wrap(card.title,y,w-pad*2,'700',69,'#ffffff',108);
    ...
    const panel=(label,content,top,tint='#ffffff')=>{const lineHeight=72,height=Math.max(218,166+lineCount(content,w-260,'600',39)*lineHeight);ctx.fillStyle=tint;ctx.beginPath();ctx.roundRect(70,top,w-140,height,28);ctx.fill();ctx.fillStyle=accent;ctx.fillRect(rtl?70:w-116,top,46,height);ctx.textAlign=rtl?'right':'left';ctx.font=font('700',27);ctx.fillStyle=accent;ctx.fillText(label,x,top+58);return wrap(content,top+132,w-260,'600',39,'#18233f',lineHeight)+44;};
    y=465; y=panel(cardLabels.rule,card.rule,y,'#ffffff')+38;
    y=panel(cardLabels.letters,card.letters,y,soft)+42;
    const casesHeader=140,casesHeight=casesHeader+card.cases.reduce((sum,group)=>sum+84+group.examples.reduce((height,example)=>height+lineCount(`• ${example}`,w-260,'500',35)*72+24,0)+30,0)+42;
    ctx.fillStyle=dark;ctx.beginPath();ctx.roundRect(70,y,w-140,casesHeight,28);ctx.fill();ctx.textAlign=rtl?'right':'left';ctx.font=font('700',29);ctx.fillStyle='#f5c372';ctx.fillText(cardLabels.examples,x,y+68);y+=casesHeader;
    card.cases.forEach(group=>{ctx.textAlign=rtl?'right':'left';ctx.font=font('700',27);ctx.fillStyle='#d5f1ef';ctx.fillText(group.label,x,y);y+=62;group.examples.forEach(example=>{y=wrap(`• ${example}`,y,w-260,'500',35,'#ffffff',72);y+=14;});y+=30;}); y+=42;
    if(card.note){const noteHeight=Math.max(190,148+lineCount(card.note,w-260,'500',34)*68);ctx.fillStyle='#fff8e8';ctx.beginPath();ctx.roundRect(70,y,w-140,noteHeight,28);ctx.fill();ctx.textAlign=rtl?'right':'left';ctx.font=font('700',28);ctx.fillStyle='#9a6515';ctx.fillText(cardLabels.note,x,y+58);y=wrap(card.note,y+122,w-260,'500',34,'#5f4818',68);}
    ctx.textAlign='center';ctx.font=font('600',26);ctx.fillStyle='#5d6682';ctx.fillText('NovaSkill Tech · 2026',w/2,h-66);
    return canvas;
  };
  async function downloadCard(card,index,format){if(document.fonts?.ready)await document.fonts.ready;const canvas=cardCanvas(card,index), type=format==='jpeg'?'image/jpeg':'image/png', extension=format==='jpeg'?'jpg':'png'; canvas.toBlob(blob=>{if(!blob)return; const link=document.createElement('a');link.href=URL.createObjectURL(blob);link.download=`tuhfat-al-atfal-card-${String(index+1).padStart(2,'0')}.${extension}`;link.click();setTimeout(()=>URL.revokeObjectURL(link.href),500);},type,.94);}
