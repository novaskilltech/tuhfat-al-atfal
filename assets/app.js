(function () {
  ...
  const renderVerse = (lesson, verse) => {
    ...
    return `<details class="verse" id="verse-${n}"><summary><span class="verse-no">${n}</span><span class="verse-text"><span>${escape(first)}</span><span>${escape(second)}</span></span><span class="chev">⌄</span></summary><div class="verse-body"><p class="quick"><strong>${u.oneLine} :</strong> ${escape(brief)}</p><h4>${u.simple}</h4><p>${escape(simple)}</p><h4>${u.detailed}</h4><p>${escape(detail)}</p><div class="details-grid"><section><h4>${u.terms}</h4><ul class="terms">${lesson.terms.map(t=>`<li>${escape(t)}</li>`).join('')}</ul></section><section><h4>${u.example}</h4><p>${escape(verseExample)}</p><p class="source-line">${u.source}</p></section><section><h4>${u.warning}</h4><p>${escape(lesson.warning)}</p></section><section><h4>${u.exercise}</h4><p>${escape(lesson.exercise)}</p></section></div><p class="source-line"><strong>${u.references} :</strong> ${escape(lessonRefs)}</p><button class="copy" data-copy="${escape(lineTitle(verse))}">${u.copy}</button></div></details>`;
  };
  ...
  const shareText = d.language==='fr' ? 'Copier pour partager' : 'نسخ للمشاركة';
  const copiedText = d.language==='fr' ? 'Copié' : 'تم النسخ';
  function textForShare(target) { const clone = target.cloneNode(true); clone.querySelectorAll('button,script,style').forEach(node=>node.remove()); return clone.innerText.replace(/\n{3,}/g,'\n\n').trim(); }
  function addShareButton(target, type) { if (target.querySelector(':scope > .share-copy')) return; const button=document.createElement('button'); button.className='share-copy'; button.type='button'; button.textContent=shareText; button.setAttribute('aria-label',shareText); button.dataset.shareType=type; target.classList.add(type==='section'?'copy-section':'copy-paragraph'); target.append(button); }
  document.querySelectorAll('main p').forEach(p=>{ if (p.closest('.copy-paragraph') || p.querySelector('button')) return; const wrap=document.createElement('div'); wrap.className='copy-paragraph'; p.before(wrap); wrap.append(p); addShareButton(wrap,'paragraph'); });
  document.querySelectorAll('main section, .lesson-group, .nuzhat-card, .flashcard').forEach(section=>addShareButton(section,'section'));
  document.addEventListener('click', async e => { const button=e.target.closest('.share-copy'); if(!button)return; const target=button.closest('.copy-paragraph,.copy-section'); try { await navigator.clipboard.writeText(textForShare(target)); button.textContent=copiedText; setTimeout(()=>button.textContent=shareText,1300); } catch { button.textContent=d.language==='fr'?'Sélectionnez le texte pour le copier.':'حدّد النص لنسخه.'; } });
